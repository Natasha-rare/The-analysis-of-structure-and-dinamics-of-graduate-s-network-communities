...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def share_csv(self):
        if self.query != "MATCH (p:Person) WHERE":
            ...
            error_dialog = QMessageBox.critical(
                self, 'Error', 'Please, select the attributes',
    ...
    def show_results_one(self, name=""):
        ...
        if len(self.result) == 0:
            ...
            error_dialog = QMessageBox.warning(
                self, 'No friends', 'Unfortunately we haven`t found friends of this graduate. Try to make other query',
                buttons=QMessageBox.Ok)
            if error_dialog == QMessageBox.Ok:
                return ''
        ...
        if self.number > 155:
            error_dialog = QMessageBox.warning(
                self, 'Large search', 'Unfortunately, your search is too large. Please, narrow the scope',
                buttons=QMessageBox.Ok)
            if error_dialog == QMessageBox.Ok:
                return ''
    ...
    def show_results(self):
        ...
        if self.query == 'MATCH (p:Person) WHERE':
            error_dialog = QMessageBox.critical(
                self, 'Empty search', 'Please, select the attributes',
                buttons=QMessageBox.Ok)
            if error_dialog == QMessageBox.Ok:
                return ''
        ...
        if len(self.result) == 0:
            error_dialog = QMessageBox.warning(
                self, 'Nothing found', "Unfortunately, we haven`t found anything.\n Try to make other query",
                buttons=QMessageBox.Ok)
            if error_dialog == QMessageBox.Ok:
                return ''
        self.number = len(self.result)
...
class Register(QtWidgets.QDialog):
    ...
    def accept(self):
        if self.name.text() == '' or self.surname.text() == '' or \
            ...
            error_dialog = QMessageBox.critical(
                self, 'Error', 'Please, fill all required fields (with *)',
                buttons=QMessageBox.Ok)
            if error_dialog == QMessageBox.Ok:
                pass
        elif self.password.text() != self.pass_repeat.text():
            error_dialog = QMessageBox.critical(
                self, 'Error', 'Passwords must be similar!!!',
                buttons=QMessageBox.Ok)
            if error_dialog == QMessageBox.Ok:
                pass
        else:
